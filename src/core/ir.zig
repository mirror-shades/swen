...
pub const IRError = reporter.Error;
...
fn lowerTransform(ir_buf: *IRBuffer, transform: types.Transform, parent_pos: types.Vector) IRError!void {
    const pos = transform.position orelse types.Vector{ .x = 0, .y = 0 };
    const world_x = transform.local_position.x + pos.x + parent_pos.x;
    const world_y = transform.local_position.y + pos.y + parent_pos.y;
